      ...
      {invoices.map((invoice, index) => {
        ...
        return (
          <Link 
            ...
            className="bg-white p-6 shadow rounded-lg block hover:border-pri-100 border border-transparent transition-colors"
          >
            {/* Mobile Layout */}
            <div className="md:hidden flex flex-col gap-6">
              ...
              <div className="flex justify-between items-center">
                <div className="flex flex-col gap-2">
                  <p className="text-sm font-medium text-sec-200">
                    Due {formattedDueDate.split(' ')[1]} {formattedDueDate.split(' ')[2]} {formattedDueDate.split(' ')[3]}
                  </p>
                  <p className="font-bold text-lg text-sec-400">
                ...
                <div className={`flex items-center justify-center gap-2 h-10 w-[104px] rounded-lg ${status === 'draft' ? 'bg-gray-100' : status === 'pending' ? 'bg-orange-100' : 'bg-green-100'}`}>
                  ...
                  <p className={`capitalize font-bold ${status === 'draft' ? 'text-gray-950' : status === 'pending' ? 'text-orange-400' : 'text-green-400'}`}>
                    {status}
                  </p>
                </div>
              </div>
            ...
            <div className="hidden md:flex items-center w-full">
              ...
              <div className="w-1/5">
                <p className="text-sm font-medium text-sec-200">{formattedDueDate}</p>
              </div>
              <div className="w-1/5">
                <p className="text-sm font-medium text-sec-300">{invoice.billTo.clientName}</p>
              </div>
              <div className="w-1/5 text-right">
                <p className="font-bold text-sec-400 text-base">
                  £ {new Intl.NumberFormat('en-GB', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                  }).format(summationItemTotal)}
              ...
              <div className="w-1/5 flex justify-end items-center gap-4">
                ...
                <IconArrowRight className="text-pri-400" />
              </div>
            </div>
          </Link>
        );
